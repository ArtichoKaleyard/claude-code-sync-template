#!/bin/bash
# Claude Code 配置快速验证脚本（Windows Bash - Git Bash/WSL）
...
WORKSPACE_PATH="${CLAUDE_WORKSPACE:-$USERPROFILE/claude-workspace}"
CLAUDECODE_ROOT="${CLAUDECODE_ROOT:-}"

path_to_hash() { echo "$1" | sed 's/:/\-/g' | sed 's|[/\\]|-|g'; }
...
echo -e "${BLUE}7️⃣  检查 .ps1 文件编码${NC}"
ps1_count=$(find "$SCRIPT_DIR" -maxdepth 1 -name "*.ps1" 2>/dev/null | wc -l)
win_script_dir=$(cygpath -w "$SCRIPT_DIR" 2>/dev/null || echo "$SCRIPT_DIR")
no_bom_files=$(powershell.exe -NoProfile -Command "
    Get-ChildItem '$win_script_dir' -Filter '*.ps1' | Where-Object {
        \$bytes = [IO.File]::ReadAllBytes(\$_.FullName)
        -not (\$bytes.Length -ge 3 -and \$bytes[0] -eq 0xEF -and \$bytes[1] -eq 0xBB -and \$bytes[2] -eq 0xBF)
    } | Select-Object -ExpandProperty Name
" 2>/dev/null | tr -d '\r')
if [ -z "$no_bom_files" ]; then
    echo -e "${GREEN}✅ 所有 .ps1 文件均为 UTF-8 BOM（共 ${ps1_count} 个）${NC}"; ((PASSED++))
else
    echo -e "${RED}❌ 以下 .ps1 文件缺少 BOM，在中文 Windows 上可能解析失败：${NC}"; ((FAILED++))
    echo "$no_bom_files" | sed 's/^/   ⚠️  /'
    echo "   💡 运行 pull.ps1 或 pull-windows.sh 可自动修复"
fi
echo ""

# 8. settings.json 同步过滤器检查
echo -e "${BLUE}8️⃣  检查 settings.json 同步过滤器${NC}"
